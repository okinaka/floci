...
public class SesService {
    ...
    public String sendEmail(SendEmailRequest request) {
        return switch (request.content()) {
            ...
            case EmailContent.Template template -> {
                EmailTemplate stored = templateService.getTemplate(template.templateName(), request.region());
                yield sendInlineTemplatedEmail(request, new EmailContent.InlineTemplate(stored.getSubject(),
                        stored.getTextPart(), stored.getHtmlPart(), template.templateData(), template.headers()));
            }
    ...
    private String sendInlineTemplatedEmail(SendEmailRequest request, EmailContent.InlineTemplate template) {
        ...
        EmailContent.Simple rendered = render(template);
        return sendSimpleEmail(request.toBuilder().content(rendered).build(), rendered);
    }

    private static EmailContent.Simple render(EmailContent.InlineTemplate template) {
        return new EmailContent.Simple(
                SesTemplateService.applyTemplateData(template.subject(), template.templateData()),
                SesTemplateService.applyTemplateData(template.textPart(), template.templateData()),
                SesTemplateService.applyTemplateData(template.htmlPart(), template.templateData()),
                template.headers());
    }
    ...
    private String sendSimpleEmail(SendEmailRequest request, EmailContent.Simple content) {
        ...
        List<String> envelope = allRecipients(request.toAddresses(), request.ccAddresses(),
        ...
        if (content.headers() != null) {
            content = new EmailContent.Simple(content.subject(), content.bodyText(), content.bodyHtml(),
                    content.headers().stream().filter(MessageHeader::isSafe).toList());
            request = request.toBuilder().content(content).build();
        }
        ...
        if (content.headers() != null && !content.headers().isEmpty()) {
    ...
    private String sendRawEmail(SendEmailRequest request, EmailContent.Raw raw) {
        String source = request.source();
        String region = request.region();
        List<String> destinations = allRecipients(request.toAddresses(), request.ccAddresses(),
                request.bccAddresses());
    ...
    public List<BulkEmailEntryResult> sendBulkEmail(SendBulkEmailRequest request) {
        ...
        for (BulkEmailEntry entry : request.entries()) {
            try {
                ...
                EmailContent.Simple rendered = render(new EmailContent.InlineTemplate(template.subject(),
                        template.textPart(), template.htmlPart(), merged, mergedHeaders));
